...
export default function CarGamePage() {
  ...
  const compositionInputRef = useRef(0);
  ...
  const handleCompositionEnd = (e) => {
    const newInputLength = e.target.value.length - startINputLengthRef.current
    compositionInputRef.current = newInputLength
  ...
  const handleInputChange = (e) => {
    ...
    const correctCount = Array.from(value).filter((char, index) => char === targetText[index]).length;
    const effectiveInputLength = compositionInputRef.current;
    const accuracy = Math.min(correctCount / effectiveInputLength, 1); 
